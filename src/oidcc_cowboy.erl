...
handle_return(Req, #state{code = AuthCode,
    ...
    of
        {ok, VerifiedToken0} ->
            ...
            {ok, RedirectUri} = oidcc_session:get_redirect_uri(Session),
            {ok, Req2} = close_session_delete_cookie(Session, Req),
            EnvMap = #{req => Req2, redirect_uri => RedirectUri},
...
redirect_user_to_provider(ProviderId, QsMap, Req, State) ->
    ...
    RedirectUri = maps:get(redirect_info, QsMap, undefined),
    ok = oidcc_session:set_redirect_uri(RedirectUri, Session),
